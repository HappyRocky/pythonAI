...
if __name__ == "__main__":
    ...
    config = configurable.Configurable(config_file=config.config_file) # 读取配置文件
    if config.cuda is True:
        print("Using GPU To Train......")
        # torch.backends.cudnn.enabled = True
        torch.backends.cudnn.deterministic = True # 用以保证实验的可重复性
        # 理论上说，已经设置了随机种子，那么初始化可以保证一致，然后整个过程应该保证一致。
        # 但是，这仅仅适用于CPU。
        # 对于GPU，还需要设置 torch.backends.cudnn.deterministic = True 才可以。
        # 如果设置torch.backends.cudnn.deterministic = True，则CuDNN卷积使用确定性算法。
        # 详情见 http://www.cnblogs.com/yongjieShi/p/9614905.html
        # https://www.ptorch.com/news/94.html
        torch.cuda.manual_seed(seed_num) # 使用单个GPU时设置随机种子
        torch.cuda.manual_seed_all(seed_num) # 使用多个GPU时设置随机种子
