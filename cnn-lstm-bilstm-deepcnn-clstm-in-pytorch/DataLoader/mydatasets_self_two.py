        ...
        if examples is None:
            ...
            with open(path) as f:
                ...
                for line in f.readlines():
                    ...
                    # 既然已经将clean_str作为text_field.preprocessing，
                    # 那么在下面data.Example.fromlist时，会自动进行 clean_str(sentence)
                    # 因此，这一句没有必要进行clean_str，否则就是重复进行clean了。
                    
                    # 二分类时，第0、1类为负样本，第3、4类为正样本
                    if line[-2] == '0':
                        a += 1
                        ...
                        b += 1
                        examples += [data.Example.fromlist([sentence, 'positive'], fields=fields)]
                    # data.Example.fromlist() 返回 Example类，其中变量 text = clean_str(sentence)，label='negative' or 'positive'
                       
                print(file + " data size: a {} b {} ".format(a, b))
    ...
    def splits(cls, path, train, dev, test, char_data, text_field, label_field, dev_ratio=.1, shuffle=True ,root='.', **kwargs):
        ...
        Arguments:
            ...
            Remaining keyword arguments: Passed to the splits method of
                Dataset.
        """
        print('train data path:' + path + train)
        print('dev data path:' + path + dev)
        print('test data path:' + path + test)
